...
[![Bundled DSH](https://img.shields.io/badge/dsh-0.1.2--rc.1-4D6BFE)](scripts/bundle-runtime.mjs)
...
- **启动 / 重启 / 停止**服务：可分别从主界面底部、设置页、托盘菜单操作；可验证归属的外部 DSH 同样支持管理，普通未知占端口程序保持保护。
- **两种拉起方式，完全自动**（无需配置）：普通版（plain）使用 `npx --yes @deepseek-ai/dsh web --no-open` 拉起 dsh 服务（自动跟随 npm 最新版；已缓存的安装复用，重启秒启——严禁 `--prefer-online`，那会每次重新下载整个依赖树导致重启卡死）；内置版（bundled，仅 `-bundled` 产物）应用自带 Node.js 与 dsh，完全离线可用。应用按构建类型自动选择。
- **自动探测 + 自动接管**：启动时探测 3080 端口——可识别的外部 DSH 直接接管并允许停止/重启；无法确认归属的普通占端口程序只复用、不操作。
- 若在「dsh 版本管理」中指定了具体版本，则始终以该版本启动（优先级最高），默认的自动拉起逻辑不再生效，详见下文「📦 dsh 版本管理」。
...
- **「退出应用时停止服务」开关**：勾选 = 托盘「退出应用」时一并停止本应用管理的服务；取消（默认）= 服务放生、继续在 3080 后台运行，下次启动自动接管。
- 可识别为 DSH 的外部服务也支持停止和重启；无法确认归属的普通占端口程序不会被本应用操作。
- 配置持久化到 `~/.dsh/bga-dsh-client/settings.json`。

### 📦 dsh 版本管理

- 设置页「dsh 版本管理」面板可查看并切换本客户端使用的 dsh 版本：**默认行为完全自动**（内置版用 bundled 运行时自带的 dsh，普通版用 npx 跟随 npm 最新版）；一旦选定某个具体版本，服务就直接以该版本启动，不再跟随 npm 最新版自动升级。
- 版本列表 = **本地已下载 + 内置版本 + npm registry 上的远程版本**，语义化倒序排列（`0.1.10 > 0.1.9`，而非字典序），当前使用的版本置顶；每行标注状态：内置 / 使用中 / 已下载 / 可下载。
- **下载**：点「下载」在后台执行 `npm install --save-exact @deepseek-ai/dsh@<版本>`，安装到 `~/.dsh/bga-dsh-client/dsh-versions/<版本>/`（每个版本约 30–50 MB）；下载过程有 toast 与「下载中…」按钮态，失败自动清理残留目录。下载用的 npm 缓存在应用独立目录（`dsh-download-cache`），不读写用户 `~/.npm`。
- **切换下载源**：可切 npm 官方源（`registry.npmjs.org`）或淘宝镜像源（`registry.npmmirror.com`），国内网络下载慢时切换即可，下次下载/刷新版本列表生效。
- **切换版本 / 恢复默认**：均需二次确认，确认后自动重启服务生效——面板会轮询等待 3080 就绪并刷新左下角的 dsh 版本显示。「恢复默认」清除选定版本：内置版回到 bundled 运行时、普通版回到 npx latest。
- **删除**：只能删除「已下载且非使用中」的版本（内置版本与正在使用的版本不给删除入口），删除在后台线程执行，按钮期间显示「删除中…」。
- 远程版本列表取自 npm registry 并**缓存 1 小时**：打开面板时若缓存过期自动后台刷新，也可点「刷新版本列表」手动拉取；**断网时仍可正常使用本地已下载与内置版本**。
- 选定版本后的启动命令形如 `<node> <dsh-versions/<版本>/…/bin.js> web --no-open`：内置版使用自带 Node.js，普通版使用系统 PATH 中的 `node`——因此**普通版要使用「版本管理」功能，本机需已安装 Node.js**。
- `dsh_version`（选定版本）与 `npm_registry`（下载源）均持久化在 `~/.dsh/bga-dsh-client/settings.json`。

### 🔄 应用更新检测

- 启动 5 秒后自动检查一次 GitHub Releases 最新版本（两次自动检查间隔 ≥ 24 小时，不频繁请求），发现新版本时设置页版本区显示「发现新版本」提示。
- 设置页左侧版本区同时展示 node / pnpm / dsh 三个工具的版本信息：服务在线时优先显示**运行中服务自报的真实版本**（npx 拉起的也能拿到；若已在「dsh 版本管理」中选定版本，则直接显示该选定版本），否则显示内置 runtime（bundled 版）或系统 PATH（plain 版）的版本；并提供「**检查更新**」按钮，可随时手动检查；检查结果实时显示：发现新版本 / 已是最新版本 / 检查失败。
...
- 客户端集成 [Sentry](https://sentry.io) 用于崩溃监控与匿名行为统计，上报内容仅限**行为事件**（应用启动、服务启动/停止、配对开关、设置保存、更新检查结果、设置页打开、dsh 版本切换、下载源切换等），详见 [docs/Sentry.md](docs/Sentry.md)。
...
1. 打开本仓库的 [Releases](../../releases/latest) 页面，在最新版本的 Assets 中下载对应你操作系统的安装包：
   - **macOS**：`.dmg` 文件，打开后拖入「应用程序」目录。
     ...
     > 🍎 **选哪个 `.dmg`？（仅 Mac 需要选择）** 先确认芯片：点击屏幕左上角苹果菜单  → **关于本机**，看「芯片」一栏。
     > - **Apple（M 系列）**：M1 / M2 / M3 / M4…（2020 年后机型）→ 下载 `*_aarch64.dmg`
     > - **Intel**：Core i3 / i5 / i7…（2020 年前机型）→ 下载 `*_x64.dmg`
...
4. 默认跟随 npm 上的最新 dsh；若想**固定在某个 dsh 版本**（不跟随最新版，或需要回退到旧版），在设置页「dsh 版本管理」中下载该版本并「切换使用」即可，随时可「恢复默认」。
...
│       ├── tray.rs               # 系统托盘
│       ├── update.rs             # 应用更新检测
│       ├── dsh.rs                # dsh 版本管理：版本列表、下载、切换、删除、下载源
│       ├── version.rs            # 版本探测与缓存（设置页左下角 node/pnpm/dsh 版本区）
