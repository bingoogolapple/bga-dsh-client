...
### 9. dsh 版本切换事件 (`dsh_active_version_set`)

用户在设置页「dsh 版本管理」中切换使用的 dsh 版本（或恢复默认）时上报。

| 字段 | 示例值 | 说明 |
|------|--------|------|
| `version` | `0.1.1-rc.2` | 切换到的 dsh 版本；点「恢复默认」清除选定版本时为 `null` |

### 10. 下载源切换事件 (`dsh_registry_set`)

用户切换 npm 下载源时上报。

| 字段 | 示例值 | 说明 |
|------|--------|------|
| `registry` | `https://registry.npmmirror.com` | 切换后的下载源地址（npm 官方源 / 淘宝镜像源） |

> 以上两个事件**只上报版本号与源地址**，不含本地安装路径、node_modules 内容等任何文件信息。
